...
public class Graph {
	...
	public void addEdge(Edge e){
		...
		if(contains(e.getPoints().substring(1,2))){
			list=map.get(e.getPoints().substring(1,2));
		}
		
		list.add(e);
		map.put(e.getPoints().substring(1,2),list);
...
public ArrayList<Edge> findQuickest(String first, String second, String placesUsed) {
		...
		ArrayList<Edge> pathTwo=new ArrayList();
		for(int i=0; i<edges.size(); i++){
			if(!placesUsed.contains(edges.get(i).getPoints().substring(0,1)) && 
					check(edges.get(i).getPoints().substring(0,1),second,placesUsed)){
					if(pathOne.size()==0){
						pathOne=findQuickest(edges.get(i).getPoints().substring(0,1),second,placesUsed);
					}	
					else{
						pathTwo=findQuickest(edges.get(i).getPoints().substring(0,1),second,placesUsed);
					}
			}
		...
		int pathTwoTime=0;
			if(pathOne.size()!=0 && pathTwo.size()!=0){
				for (int i=0; i<pathOne.size();i++)
				{
					pathOneTime+=pathOne.get(i).getTime();
				}
				
				for (int i=0; i<pathTwo.size();i++)
				{
					pathTwoTime+=pathTwo.get(i).getTime();
				}
				System.out.println(pathOneTime+" 2 "+pathTwoTime);
				for (Edge q : pathOne)
					System.out.print(q + " ");
				System.out.println();
				for (Edge q : pathTwo)
