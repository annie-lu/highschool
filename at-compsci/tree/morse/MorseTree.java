...
public class MorseTree
{
	...
	public Morse balance(String arr[])
	{
		return balance(arr, 0, arr.length-1);
	}
	
	private Morse balance(String[] arr, int start, int end)
	{
	  if (start > end)
	  {
		  return null;
     ...
     Morse newNode = new Morse(arr[middle],middle);
	...
	private int getNumLevels(Morse tree)
	{
		if(tree==null)
			return 0;
	...
	private void display(Morse tree, int level)
	{
		if (tree==null)
			System.out.println("");
	else if(level==0)
	...
	private String reverseTranslate(Morse node, int index, String trans)
	{
		if (node == null)
			return trans;
	...
	private Morse translate(String morse, int index, Morse node)
	{
	...
	public static void main(String[] args)
	{
		...
		System.out.println(tree.reverseTranslate("MY FEET SMELL AND MY NOSE RUNS"));
