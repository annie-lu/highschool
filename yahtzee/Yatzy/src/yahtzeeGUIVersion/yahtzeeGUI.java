...
public class yahtzeeGUI implements ActionListener {
	...
	private JTextField txt_p1_GTotal, txt_p2_GTotal;
	private JTextField txt_p1_YahtzeeBonus;
	private JTextField txt_p2_YahtzeeBonus;
	Player player1;
	Player player2;
	...
	public yahtzeeGUI() {
		...
		player1 = gameLogic.getPlayerOne();
		player2 = gameLogic.getPlayerTwo();
	...
	private void createMainFrameElements()
	{
		...
		lblGrandTotal.setBounds(250, 378, 107, 14);
		frmYahtzee.getContentPane().add(lblGrandTotal);
		
		txt_p1_GTotal = new JTextField();
		txt_p1_GTotal.setBounds(368, 375, 45, 20);
		frmYahtzee.getContentPane().add(txt_p1_GTotal);
		txt_p1_GTotal.setText("0");
		txt_p1_GTotal.setName("txt_p1_GTotal");
		txt_p1_GTotal.setHorizontalAlignment(SwingConstants.CENTER);
		txt_p1_GTotal.setEditable(false);
		txt_p1_GTotal.setColumns(10);
		txt_p1_GTotal.setBackground(Color.WHITE);
		
		txt_p2_GTotal = new JTextField();
		txt_p2_GTotal.setText("0");
		txt_p2_GTotal.setName("txt_p2_GTotal");
		txt_p2_GTotal.setHorizontalAlignment(SwingConstants.CENTER);
		txt_p2_GTotal.setEditable(false);
		txt_p2_GTotal.setColumns(10);
		txt_p2_GTotal.setBackground(Color.WHITE);
		txt_p2_GTotal.setBounds(420, 375, 45, 20);
		frmYahtzee.getContentPane().add(txt_p2_GTotal);
		
		JLabel lblYahtzeeBonus = new JLabel("Yahtzee Bonus");
		lblYahtzeeBonus.setHorizontalAlignment(SwingConstants.RIGHT);
		lblYahtzeeBonus.setBounds(250, 348, 107, 14);
		frmYahtzee.getContentPane().add(lblYahtzeeBonus);
		
		txt_p1_YahtzeeBonus = new JTextField();
		txt_p1_YahtzeeBonus.setText("0");
		txt_p1_YahtzeeBonus.setName("txt_p1_YahtzeeBonus");
		txt_p1_YahtzeeBonus.setHorizontalAlignment(SwingConstants.CENTER);
		txt_p1_YahtzeeBonus.setEditable(false);
		txt_p1_YahtzeeBonus.setColumns(10);
		txt_p1_YahtzeeBonus.setBackground(Color.WHITE);
		txt_p1_YahtzeeBonus.setBounds(368, 345, 45, 20);
		frmYahtzee.getContentPane().add(txt_p1_YahtzeeBonus);
		
		txt_p2_YahtzeeBonus = new JTextField();
		txt_p2_YahtzeeBonus.setText("0");
		txt_p2_YahtzeeBonus.setName("txt_p2_YahtzeeBonus");
		txt_p2_YahtzeeBonus.setHorizontalAlignment(SwingConstants.CENTER);
		txt_p2_YahtzeeBonus.setEditable(false);
		txt_p2_YahtzeeBonus.setColumns(10);
		txt_p2_YahtzeeBonus.setBackground(Color.WHITE);
		txt_p2_YahtzeeBonus.setBounds(420, 345, 45, 20);
		frmYahtzee.getContentPane().add(txt_p2_YahtzeeBonus);
	}
	...
	private void createDice(){
		...
		rdbtnHidden.setBounds(204, 21, 28, 23);
		diceBox.add(rdbtnHidden);
	...
	private void createUpperScoreboard()
	{
		...
		separator_U1.setBounds(6, 197, 190, 12);
	...
	private void createLowerSection(){
		...
		lowerSection.setBounds(240, 65, 237, 274);
	...
	private void createLowerScoreboard()
	{
		...
		separator_L1.setBounds(6, 225, 220, 6);
	...
	private void confirmSelection()
	{
		...
		for (int radioIndex = 0; radioIndex < rdbtnNames.length-1; radioIndex++)
		{
			...
			if(button.isSelected())
				gameLogic.getCurrentPlayer().setAction(radioIndex,false); 
	...
	{
		int currentPlayer = gameLogic.getCurrentPlayerNumber();
		
		for (int radioIndex = 0; radioIndex < rdbtnNames.length-1; radioIndex++)
		{
			JRadioButton button = ((JRadioButton) getComponentByName(rdbtnNames[radioIndex]));
			boolean[] actions = gameLogic.getCurrentPlayer().getAllowedActions();
	...
	{
		if(gameLogic.getCurrentPlayerNumber() == 1)
		{
	...
	private void saveScores()
	{
		for (int radioIndex = 0; radioIndex < rdbtnNames.length-1; radioIndex++)
		{
			...
			if (gameLogic.getCurrentPlayerNumber() == 1) {
				...
				gameLogic.getCurrentPlayer().addScore(rdbtnNames[radioIndex].substring(5), p1Score);
			} else {
				String p2Str = "txt_p2_" + rdbtnNames[radioIndex].substring(5);
				JTextField p2Field = ((JTextField) (getComponentByName(p2Str)));
				...
				gameLogic.getCurrentPlayer().addScore(rdbtnNames[radioIndex].substring(5), p2Score);
			}
	...
	private void displayTotals()
	{
		...
		if(gameLogic.getCurrentPlayer().equals(player1)){
			...
			((JTextField)getComponentByName("txt_p1_YahtzeeBonus")).setText(""+player1.getYahtzeeBonus());
		}
		else{
			...
			((JTextField)getComponentByName("txt_p2_YahtzeeBonus")).setText(""+player2.getYahtzeeBonus());
